...
reference commit id"""
    ...
    def test_no_repo(self):
        ...
        """
        from build_node.utils.git_utils import git_get_commit_id, GitCommandError
...
class TestGitLsRemote(GitUtilsShellTest):
    ...
    def test_tags(self):
        ...
        self.__test_git_refs(tags, 'tag',
                             'ssh://user@example.com:29418/alt-php70')

    def test_heads(self):
        """git_ls_remote returns heads"""
        heads = [
            ('93182f76f55a7967f77372a06abed6a00bf646f9', 'master', 'head'),
            ('9708ca946f80960ad9bb5447fb2a494b95f776d0', 'refactoring-ui',
             'head')
        ]
        self.__test_git_refs(heads, 'head',
                             'ssh://user@example.com:29418/alt-php70')
    ...
    def test_not_git_repo(self):
        ...
        sys.exit(128)
        """
        self.__test_git_command_error(user_code, '/test/not-a-repo',
    ...
    def test_project_not_found(self):
        ...
        self.__test_git_command_error(user_code, repo_path, 'Project not found')

    def __test_git_command_error(self, user_code, repo_path, error_message):
        with MockShellCommand('git', textwrap.dedent(user_code)):
            from build_node.utils.git_utils import git_ls_remote, GitCommandError
    ...
    def __test_git_refs(self, refs, ref_type, repo_path):
        user_code = '\n'.join(['print("{0}\trefs/{2}s/{1}")'.format(*r)
                               for r in refs])
        with MockShellCommand('git', user_code) as cmd:
            from build_node.utils.git_utils import git_ls_remote
            result_refs = git_ls_remote(repo_path,
                                        **{'{0}s'.format(ref_type): True})
            self.assertEqual(refs, result_refs)
            call = cmd.get_calls()[0]
            ref_arg = '--{0}s'.format(ref_type)
            self.assertTrue(ref_arg in call['argv'][1:],
                            msg='git must be called with {0} argument'.
                            format(ref_arg))
            self.__verify_git_args(call, repo_path)

    def __verify_git_args(self, call, repo_path):
        self.assertDictContainsSubset({'LANG': 'C'}, call['env'],
                                      msg='git must be called with C locale')
        self.assertEqual(call['argv'][1], 'ls-remote',
                         msg='first git argument must be "ls-remote"')
        self.assertEqual(call['argv'][-1], repo_path,
