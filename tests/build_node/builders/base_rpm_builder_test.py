...
class TestConfigureMockNpmProxy(TestCase):
    ...
    def __get_file_option(self, config, file_name):
        ...
        """
        regex = r'config_opts\["files"\]\["{0}"\]\s*?=\s*?"""(.*?)"""'.format(
            file_name
        )
        re_rslt = re.search(regex, config, flags=re.MULTILINE | re.DOTALL)
        self.assertIsNotNone(
            re_rslt, '{0} mock file is not found'.format(file_name)
        )
    ...
    def __validate_npm_config(self, npm_config):
        ...
        regexes = (
            r'https-proxy\s*?=\s*?{0}'.format(self.npm_proxy),
            r'proxy\s*?=\s*?{0}'.format(self.npm_proxy),
            r'strict-ssl\s*?=\s*?false',
        )
        for regex in regexes:
            self.assertIsNotNone(
                re.search(regex, npm_config, flags=re.MULTILINE),
    ...
    def __validate_yarn_config(self, yarn_config):
        ...
        regexes = (
            r'https-proxy\s+?"{0}"'.format(self.npm_proxy),
            r'proxy\s+?"{0}"'.format(self.npm_proxy),
            r'strict-ssl\s+?false',
        )
        for regex in regexes:
            self.assertIsNotNone(
                re.search(regex, yarn_config, flags=re.MULTILINE),
                '{0} option is not found'.format(regex),
            )


@pytest.mark.skip(reason="We need to rewrite it to pytest")
...
project name
    ...
    def test_first_found_spec(self):
        ...
        for i in range(3):
            with open(
                os.path.join(self.test_dir, 'random.{0}.spec'.format(i)), 'w'
            ) as fd:
...
class TestIsSrpmBuildExcluded(TestCase):
    def setUp(self):
        ...
        self.mock_error = MockError(
            'mock', 1, 'stdout', 'stderr', None, resultdir=self.test_dir
        )
...
architecture"""
        with open(self.build_log_path, 'w') as fd:
            fd.write(
                'BUILDSTDERR: Mock Version: 1.4.9\nBUILDSTDERR: error: '
                'No compatible architectures found for build\nBuilding '
                'target platforms: i686\n'
            )
        excluded, reason = BaseRPMBuilder.is_srpm_build_excluded(
            self.mock_error
        )
...
class TestSaveBuildArtifacts(TestCase):
    ...
    def test_saves_mock_build_log(self):
        ...
        self.__test_saves_file(
            r'^mock_build{0}\.\d+\.log$', self.mock_build_log
        )

    def test_saves_mock_root_log(self):
        """BaseRPMBuilder.save_build_artifacts saves mock root log"""
        self.__test_saves_file(r'^mock_root{0}\.\d+\.log$', self.mock_root_log)
    ...
    def __validate_timestamp(self, file_name):
        ...
        if re_rslt:
            ...
            self.assertTrue(
                0 <= delta <= 30, '{0} timestamp is invalid'.format(file_name)
            )
...
class TestIsBuildExcluded(TestCase):
    ...
    def setUp(self, build_task):
        ...
        self.builder = BaseRPMBuilder(Mock(), None, task, None, None, None)
    ...
    def test_exclusive_arch(self, extract_metadata):
        ...
        self.assertRegex(
            message, r'architecture is not listed in ExclusiveArch'
        )
    ...
    def test_normal_build(self, extract_metadata):
        ...
        self.assertEqual(
            self.builder.is_build_excluded('test.rpm'), (False, None)
        )
...
class TestIsKojiSources(TestCase):
    ...
    def test_koji_builder(self):
        ...
        task = self.__generate_task(
            'KojiBuilder', 'cla.build_system.builders.koji_builder'
        )
    ...
    def test_alt_php_builder(self):
        ...
        task = self.__generate_task(
            'AltPHPBuilder', 'cla.build_system.builders.alt_php_builder'
        )
        self.assertTrue(BaseRPMBuilder.is_koji_sources(self.sources_dir, task))
    ...
    def test_cl6_lve_kernel_kmod_builder(self):
        ...
        task = self.__generate_task(
            'CL6LveKernelKmodBuilder', 'cla.build_system.builders.kernel'
        )
        self.assertTrue(BaseRPMBuilder.is_koji_sources(self.sources_dir, task))
    ...
    def test_cl_package_builder(self):
        ...
        task = self.__generate_task(
            'CLPackageBuilder', 'cla.build_system.builders.cl_package_builder'
        )
        self.assertFalse(
            BaseRPMBuilder.is_koji_sources(self.sources_dir, task)
        )
    ...
    def __generate_task(self, builder_class, builder_module):
        return {
            'build': {
                'builder': {'class': builder_class, 'module': builder_module}
            }
        }
...
class TestIsSrpmBuildRequired(TestCase):
    ...
    @pytest.fixture(autouse=True)
    def test_srpm_in_task(self, build_task_src_rpm):
        """
        BaseRPMBuilder.is_srpm_build_required returns False for existent src-RPM
        """
        self.assertFalse(build_task_src_rpm.is_srpm_build_required())

    @pytest.fixture(autouse=True)
    def test_no_srpm(self, build_task):
        """
        BaseRPMBuilder.is_srpm_build_required returns True for missing src-RPM
        """
        self.assertTrue(build_task.is_srpm_build_required())
...
class TestUnpackSources(TestCase):
    ...
    @pytest.fixture(autouse=True)
    def setUp(self, build_task_src_rpm):
        self.setUpPyfakefs()
        self.config = Mock()
...
development mode"""
    ...
    def __check_src_dir_exists(self, src_dir):
        self.assertTrue(
            os.path.isdir(src_dir), 'src-RPM sources directory is not created'
        )
    ...
    def __check_unpack_src_rpm_call(self, unpack_src_rpm):
        unpack_src_rpm.assert_called_with(
            self.srpm_path, os.path.dirname(self.srpm_path)
        )
