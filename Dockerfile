...
RUN dnf install -y --enablerepo="powertools" --enablerepo="epel" --enablerepo="buildnode" \
    ...
    python3-pycurl libicu libicu-devel python3-pymongo python3-pymongo-gridfs \
    python3-lxml git tree mlocate mc createrepo_c python3-createrepo_c xmlsec1-openssl-devel \
    kernel-rpm-macros python3-libmodulemd dpkg-dev mock debootstrap pbuilder apt apt-libs \
    python3-apt npm keyrings-filesystem ubu-keyring debian-keyring raspbian-keyring qemu-user-static
